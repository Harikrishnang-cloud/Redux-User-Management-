...
function DashBoard() {
  ...
  useEffect(()=>{
    ...
    try {
      ...
      console.log(respones.data.usersList)
      setUser(respones.data.usersList || [])
    } catch (error) {
      console.log(error)
    }
  }
 ...
 const saveEdit=async()=>{
  ...
  try {
    ...
    const res=await Api.put(`/user/editProfile/${editUser._id}`,updateData)
    setUser(user.map((u)=>(u._id===editUser._id ? res.data.update :u)))
              ...
              <tbody>
                {filteredUsers.map((u)=>(
                 <tr key={u._id} className=" hover:bg-gray-50">
                    ...
                    <img src={u.profileImage || "No photo"} alt="Profile" className="w-10 h-10 rounded-full object-cover"/></td>
